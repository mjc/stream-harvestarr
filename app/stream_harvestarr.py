    ...
    def get(self, ydl_opts, playlist):
        ...
        if key not in self.refreshed:
            ...
            if fresh_entries is not None:
                ...
                logger.warning(
                    'Source refresh failed for %s; %s', playlist,
                    'using the previous complete snapshot' if key in self.entries
...
class StreamHarvester:
    ...
    def find_episode(self, series, episode):
        ...
        if search_url and series.get('channel_search', False):
            ...
            search_options.update({
                ...
                # Search ranks by relevance; only the source fallback should
                # honour the user's chronological ordering preference.
    ...
    def download_video(self, url, options, title):
        ...
        try:
            ...
            if not ((options.get('writesubtitles') or options.get('writeautomaticsub'))
